...
const CommentList = ({ comments, pageNum = 1, totalPages = 0, onClickPage = () => {} }) =>
  <div>
  <ul class="list-group">
  {
    comments.map((comment) =>
      <li key={comment.id} class="list-group-item">
        <div class="row align-items-center gx-2">
          ...
          <div class="col">
            <h5 class="mb-0">
              <Link to={`/${comment.id}`}>COMMENT {comment.id}</Link>
            </h5>
            <ul class="list-inline list-separator small">
              <li class="list-inline-item">{ JSON.stringify(comment).substring(0,70) + "…" }</li>
              <li class="list-inline-item">{ new Date(comment.createdAt).getDate() + "." + (new Date(comment.createdAt).getMonth() + 1) + "." + new Date(comment.createdAt).getFullYear() }</li>
          ...
          <div class="col-auto">
            {/* Options */}
            <Link to={`/${comment.id}`} className="btn btn-sm btn-white">
